...
$sql="SELECT 
  `tm_user`.`fio`,
  `tm_user`.`num`,
  COUNT(`tm_chat_kurator`.`u_pr`) AS cupr
FROM
  `tm_chat_kurator`
  INNER JOIN `tm_user` ON (`tm_chat_kurator`.`user` = `tm_user`.`num`)
WHERE
  `tm_chat_kurator`.`kurator` = $user
GROUP BY
...
$sql="update  `tm_chat_kurator` set u_pr=NULL where user=$users";
...
FROM
  `tm_chat_kurator`
WHERE
  `tm_chat_kurator`.`kurator` = $user AND 
  `tm_chat_kurator`.`user` = $users AND `tm_chat_kurator`.`razdel`=".$_SESSION['rzd']." order by num desc limit 50) AS t1 
ORDER BY num;";	
...
if ((isset($_POST["chat"])) && ($_POST["chat"] == "1")) {
	$users=intval($_POST["user"]);
	$txt=$_POST["txt"];
$sql="INSERT INTO `tm_chat_kurator` (`num`, `user`, `kurator`, `razdel`, `txt`, `dat`, `ku`,`k_pr`, `u_pr`) VALUES (NULL, $users, ".$_SESSION['MM_Username2020'].",".$_SESSION['rzd'].", '$txt', '".date("Y-m-d H:i:s")."',1,1, NULL)";
//echo $sql;
	DB::Query($sql, $testmed) or die( /* fixed MMiC */ mysqli_error(DB::$link));
$sql="SELECT * FROM (SELECT 
  `tm_chat_kurator`.`num`,
...
WHERE
  `tm_nmo_razd`.`prepod` =$user and `tm_nmo_razd`.id=$rr
union
SELECT DISTINCT 
  `tm_user`.`num`,
  `tm_user`.`fio`
...
<script src="js/bootstrap.js"></script>
  ...
  <script type="text/javascript">
	$(function() {
		...
		function ref(){
				$.post('kurator_forum.php', {'chat':'3', 'user' :defuser,'txt':''},
		function(data) {
			$('#npr').html(data);
			...
			$.post('kurator_forum.php', {'chat':'2', 'user' :defuser,'txt':''},
		function(data) {
			$('.chat').html(data);
