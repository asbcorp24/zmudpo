...
$st=(int)$_GET['stud'];
  ...
  `tm_spec`.`nazv`,`tm_spec`.`chas`,`tm_spec`.`dat`
FROM
  `tm_user`
...
FROM
  `tm_typsv_konf_user`
WHERE
  `tm_typsv_konf_user`.`ank` = 14 AND 
  `tm_typsv_konf_user`.`user` = $st";
  
$stud1 =  /* fixed MMiC */ DB::Query($sql, $testmed) or die( /* fixed MMiC */ mysqli_error(DB::$link));
$row_stud1 =  /* fixed MMiC */ mysqli_fetch_assoc($stud1);
...
$ttfImg->setFont('./times.ttf', 60, "#880d0d", 0); 
$ttfImg->text_center("183542".$row_stud['num'],230,730,900);


$ttfImg->setFont('./times.ttf', 40, "#576860", 0);      
$ttfImg->text_center("Принял участие в конференции",150,1100,1200);

$ttfImg->setFont('./times.ttf',50, "#576860", 0); 
$message = $ttfImg->textFormat(1200, 500, 
 $row_stud['nazv']);
$ttfImg->text_center($message,10,1200,1390);


$ttfImg->setFont('./times.ttf',30, "#576860", 0); 
$message = $ttfImg->textFormat(500, 500, 
 'Директор ГАПОУ "Зеленодольское медицинское училище"');
$ttfImg->text_center($message,80,1700,500);


$ttfImg->setFont('./times.ttf',30, "#576860", 0); 
$message = $ttfImg->textFormat(500, 500, 
 'Р.В. Латыпов');
$ttfImg->text_center($message,900,1700,500);

 
$ttfImg->setFont('./times.ttf', 40, "#576860", 0); 
$message = $ttfImg->textFormat(800, 500, 
 $row_stud1['value']);
$ttfImg->text_center('Тема статьи: '.$message,10,1300,1390);


$ttfImg->text_center($row_stud['dat'],10,1860,1390);
