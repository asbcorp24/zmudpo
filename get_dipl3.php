...
if (isset($_GET['stud'])){
  ...
  `ts_arh_stud`.`inn`,
  `ts_arh_stud`.`nreg`,
  `ts_arh_stud`.`datav`,`ts_arh_stud`.`protocol`
FROM
  `ts_arh_stud`
WHERE
...
$ttfImg = new ttfTextOnImage('sv_odol.jpg');
      
// Пишем шрифтом Scrawn размером 64 пункта бордовым цветом с 80%-ой прозрачностью 

$ttfImg->setFont('timesbd.ttf', 25, "#e6878b", 0);      
$ttfImg->text_center("771800986".$row_stud['num'],50,530,680); 
$ttfImg->text_center($row_stud['crasn_reg'],10,745,680); 

 $ttfImg->setFont('timesbd.ttf', 20, "#576860", 0);
$ttfImg->text_center($row_stud['nreg'],20,680,700);
$ttfImg->setFont('times.ttf', 20, "#576860", 0); 
$ttfImg->text_center("город",180,1150,800);
 $ttfImg->setFont('timesbd.ttf', 20, "#576860", 0);
$ttfImg->text_center("Зеленодольск",10,840,750);


 $ttfImg->setFont('timesbd.ttf', 20, "#576860", 0);
$ttfImg->text_center($row_stud['datav'],10,760,700);


$ttfImg->setFont('times.ttf', 20, "#576860", 0);      
//$ttfImg->text_center("Настоящее удостоверение свидетельствует о том, что",1300,220,800);
$ttfImg->setFont('times.ttf', 30, "#576860", 0);    
$message = $ttfImg->textFormat(600, 200, 
 $row_stud['fam']." ".$row_stud['name']." ".$row_stud['otch']);
$ttfImg->text_center($message,550,180,900);
...
$ttfImg->text_center($message,50,250,700);

$ttfImg->setFont('times.ttf', 20, "#576860", 0); 
$message = $ttfImg->textFormat(500, 200, 
 $row_spec0['naz']);
$ttfImg->text_center($message,550,310,900);
...
$ttfImg->setFont('times.ttf', 20, "#576860", 0); // $ttfImg->setFont('timesbd.ttf', 20, "#000000", 0);
...
$stamp = imagecreatefrompng('P_s.png');
$xx=990;
$yy=750;
//$stamp = imagerotate($stamp, 20, 0);
imagecopy($ttfImg->hImage, $stamp, $xx, $yy, 0, 0, imagesx($stamp), imagesy($stamp));
...
$stamp = imagecreatefrompng('pech2.png');
$xx=1050;
$yy=700;
//$stamp = imagerotate($stamp, 20, 0);
imagecopy($ttfImg->hImage, $stamp, $xx, $yy, 0, 0, imagesx($stamp), imagesy($stamp));
