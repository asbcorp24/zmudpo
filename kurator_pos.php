...
$sql="SELECT 
  `tm_user`.`fio`, `tm_user`.`num`,  `tm_grupp`.`nazv` as value
FROM
  `tm_user`
  INNER JOIN `tm_nmo_razd` ON (`tm_user`.`spec` = `tm_nmo_razd`.`spec`)
    LEFT OUTER JOIN `tm_grupp` ON (`tm_user`.`grupp` = `tm_grupp`.`id`)
WHERE
  `tm_nmo_razd`.`id` =	$media";
	//echo $sql;
$media2 =  /* fixed MMiC */ DB::Query($sql, $testmed) or die( /* fixed MMiC */ mysqli_error(DB::$link));
$row_media2 =  /* fixed MMiC */ mysqli_fetch_assoc($media2);
$totalRows_media2 =  /* fixed MMiC */ mysqli_num_rows($media2);

//data-id="grp echo $row_media['value']; "	
echo '<div class="table-responsive"> <table width="200" border="1"  class="table table-bordered table-striped" id="addra">';
 echo ' <tbody>';
echo '    <tr>';
echo '      <th scope="col">группа</th>';
echo '      <th scope="col">фио</th>';
 
echo '    </tr>';
	   do { 
		   echo '    <tr  id="ttr'.$row_media2['utnum'].'"  data-id="grp'.$row_media2['value'].'" class="fuser">';
		    echo '<td>';
		   echo "<h3>".$row_media2['value']."</h3> ";
		   echo '</td>';
		    echo '<td>';
		   echo "<h3>".$row_media2['fio']."</h3> ";
		   echo '</td>';
		   $sql="SELECT DISTINCT 
  `nmo_otm_pos`.`dat`
FROM
  `nmo_otm_pos`
WHERE
  `nmo_otm_pos`.`razdel` = $media and  `nmo_otm_pos`.`user` = ".$row_media2['num']."
ORDER BY
  `nmo_otm_pos`.`dat`";
  $media3 =  /* fixed MMiC */ DB::Query($sql, $testmed) or die( /* fixed MMiC */ mysqli_error(DB::$link));
$row_media3 =  /* fixed MMiC */ mysqli_fetch_assoc($media3);
$totalRows_media3 =  /* fixed MMiC */ mysqli_num_rows($media3);
 do {
 		$d=strtotime($row_media3['dat']);
 	echo  ' <td scope="col" class="fuser dtr"  data-ide="'.date("Y-m-d",$d).'">'.$row_media3['dat'].'</td>';
				  } while ($row_media3 =  /* fixed MMiC */ mysqli_fetch_assoc($media3));
...
echo "</td><tr>";
				  } while ($row_media2 =  /* fixed MMiC */ mysqli_fetch_assoc($media2));
  ...
  <div class="panel panel-default">
    ...
    <div class="panel-heading">
      <h4 class="panel-title">
		  ...
		  <select name="media">
		   <?php do { ?>
  <option value="<?php echo $row_stud2a['id']; ?>"> <?php echo getsokr($row_stud2a['nazv']); ?></option>
		...
		<button class="fdgrp" data-id="<?php echo $row_spec['id']; ?>" >фильтр</button>
		<span class=""><button type="button" class="btn btn-primary btn-xs pull-right" id="pch2"><i class="glyphicon glyphicon-print"></i></button></span>
  ...
  <script type="text/javascript">
	...
	$('.dtr').each(function(idx){//=idx;
						...
						if ($(this).data('ide')==deff) {$(this).show();
									    console.log("="+$(this).data('ide'),deff);
									   } else $(this).hide();;		   
