...
if (!function_exists(' /* fixed MMiC */ true;//mysql_connect') || !function_exists('mysql_pconnect') || !extension_loaded('mysql')){
...
*/
	...
	function Open()
	{
	  $this->connectionId = /* fixed MMiC */ true;//mysql_connect($this->hostname, $this->username, $this->password);
		if (isset($this->connectionId) && $this->connectionId && is_resource($this->connectionId))
		{
			$this->isOpen = ($this->database == "") ? true :  /* fixed MMiC */ mysqli_select_db(DB::$link, $this->database);
		}
		else
		{
			$this->isOpen = false;
	...
	function Close()
	{
		...
		{
			if ( /* fixed MMiC */ mysqli_close(DB::$link))
			{
				$this->isOpen = false;
	...
	function GetTables($table_name = '')
	{
		...
		if ($this->isOpen && isset($this->connectionId) && is_resource($this->connectionId)){
			...
			$results =  /* fixed MMiC */ DB::Query($sql, $this->connectionId) or $this->HandleException();
			...
			if (is_resource($results) &&  /* fixed MMiC */ mysqli_num_rows($results) > 0){
					while ($row =  /* fixed MMiC */ mysqli_fetch_array($results)){
	...
	function GetColumnsOfTable($TableName)
	{
		...
		$result =  /* fixed MMiC */ DB::Query($query) or $this->HandleException();
		...
		if ($result)
		{
			...
			while ($row =  /* fixed MMiC */ mysqli_fetch_array($result, MYSQL_ASSOC))
			{
			 ...
			 /* fixed MMiC */ mysqli_free_result($result);
	...
	function ExecuteSQL($aStatement, $MaxRows)
	{
		...
		$result =  /* fixed MMiC */ DB::Query($aStatement) or $this->HandleException();
		
		if (isset($result) && is_resource($result))
		{
			$xmlOutput = "<RESULTSET><FIELDS>";

			$fieldCount =  /* fixed MMiC */ mysqli_num_fields($result);
			for ($i=0; $i < $fieldCount; $i++)
			{
				$meta =  /* fixed MMiC */ mysqli_fetch_field($result);
				if ($meta)
				{
					$xmlOutput .= '<FIELD';
			...
			$xmlOutput .= "</FIELDS><ROWS>";
			$row =  /* fixed MMiC */ mysqli_fetch_assoc($result);
	...
	*/
	function HandleException()
	{
		...
		$this->error = create_error(' MySQL Error#: '. ((int) /* fixed MMiC */ mysqli_errno(DB::$link)) . "\n\n". /* fixed MMiC */ mysqli_error(DB::$link));
	...
	function GetDatabaseList()
	{
		...
		if (isset($this->connectionId) && is_resource($this->connectionId)){
				$dbList =  /* fixed MMiC */ DB::deprecated('mysql_list_dbs');//mysql_list_dbs($this->connectionId);
				
				while ($row =  /* fixed MMiC */ mysqli_fetch_object($dbList))
				{
