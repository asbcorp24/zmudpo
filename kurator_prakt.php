...
FROM
  `tm_nmo_pract`
WHERE
  `tm_nmo_pract`.`razdel` = $media AND 
  `tm_nmo_pract`.`user` = $id
ORDER BY
  `tm_nmo_pract`.`dat`";
...
WHERE
  `tm_nmo_razd_media`.`id` = $media AND 
  (`tm_nmo_razd_dop_prepod`.`prepod` = $prepod OR 
  `tm_nmo_razd`.`prepod` = $prepod)"	;
		$spec08 =  /* fixed MMiC */ DB::Query($sql, $testmed) or die( /* fixed MMiC */ mysqli_error(DB::$link));
$row_spec08 =  /*81 =  /* fixed MMiC */ mysqli_num_rows($spec08);
//echo $sql;	
?>
...
<div class="panel-group" id="accordion">
  <!-- 1 панель -->
	  <?php do { ?>
	    <tr>
   <td><?php if ($row_spec08>0){ ?><button data-id="<?php echo $row_spec01['id']; ?>" class="sho" data-d="y"><i class="glyphicon glyphicon-minus"></i></button>
	   <?php } ?></td>
      <td><?php echo $row_spec01['dat']; ?></td>
...
$sql="SELECT DISTINCT 
  ...
  `tm_grupp`.`nazv`,`tm_nmo_razd_media`.`id`
FROM
  `tm_nmo_razd`
...
$sql="SELECT DISTINCT 
  `tm_nmo_razd`.`nazv`,
  `tm_nmo_razd_media`.`id` as pid,`tm_nmo_razd_media`.`nazv` as mnazv,
    `tm_nmo_razd`.`id`
FROM
  `tm_nmo_razd_media`
  INNER JOIN `tm_nmo_razd` ON (`tm_nmo_razd_media`.`tm_nmo_razd` = `tm_nmo_razd`.`id`)
...
<script src="js/bootstrap.js"></script>
 ...
 $('body').on('show.bs.collapse', '.bfd', function(){
   id=$(this).data('id');
	...
	$.post( "kurator_prakt.php",{'get':1,'id':id,'media':media}, function(data) {
$('#fss'+id).html(data);
...
id=$('#srazd').val();
$.post( "kurator_prakt.php",{'fio':1,'id':id}, function(data) {
$('#todo').html(data);
