...
$sql="SELECT DISTINCT 
  ...
  `tm_nmo_razd_media`.`nazv`,`tm_nmo_razd_media`.`comment`
FROM
  `tm_nmo_razd_media`
WHERE
  `tm_nmo_razd_media`.`tm_nmo_razd` = $media AND 
  (`tm_nmo_razd_media`.`tip` = 18)";
...
echo '    </tr>';
	   do { 
        ...
        echo '<button type="button" class="btn btn-primary btn-xs pull-right rpc3" data-im="'.$row_media['media_id'].'">E</button>';
...
$rtotalRows_media =  /* fixed MMiC */ mysqli_num_rows($rmedia);   
	...
	echo '	   <table width="200" border="1" class="table table-bordered table-striped" id="'.$row_media['media_id'].'">';
 echo ' <tbody>';
 echo '   <tr >';
...
echo ' 			<td align="center" class="npc"><button data-razdel="'.$rrow_media['razdel'].'" class="pank" data-user="'.$rrow_media['num'].'" data-id="'.$rrow_media['utnum'].'" data-path="'.$row_media['comment'].'"><i class="glyphicon glyphicon-asterisk"></i></button></td>';
echo '      <td id="fio">'.$rrow_media['fio'].'</td>';
...
WHERE
 `tm_user`.`num`=".$rrow_media['num']." and `tm_nmo_razd_user_arh`.`dat`<>'".$rrow_media['dat']."' and 
  `tm_nmo_razd_user_arh`.`razdel` = ".$row_media['media_id'];;	
...
if ($rtotalRows_media1 >0 ){
 echo '<tr  data-id="grp'.$rrow_media['value'].'" class="fuser dtr"  data-ide="'.date("Y-m-d", $d).'""><td colspan=6>';
do {
...
<script src="js/bootstrap.js"></script>
 ...
 <script src="js/FileSaver.min.js"></script>
  <script type="text/javascript">
	...
	ts=	$(this);	
		$(this).hide();
			$(this).parent().parent().printThis({ canvas: true,afterPrint:function(){$(ts).show();}});
		//	$('.npc').show();
	});	
		$('body').on('click', '.rpc2',function(){
	ts=	$(this);	
		$(this).hide();
			$(this).parent().parent().parent().printThis({ canvas: true,afterPrint:function(){$(ts).show();}});
		//	$('.npc').show();
	});	
...
num=$(this).data('id')	;
$.get( "kurator_ank.php",{'num':num,'dat':1}, function(data) {
...
$('#tr1_'+num).show();
 // alert( "success" );
  ...
  $('body').on('click', '.rpc3',function(e){
  im=$(this).data('im');
    $('#ttn').empty();
fl6=2;
$('#'+im+' .dtr td .pank').each(function(ind,val){

fio=$(val).parent().siblings('#fio').html();
$('footer').append('<table id="ttn" border="1"><table>');
s='http://zmudpo.ru/nmo/obr/'+$(val).data('path')+'?path='+$(val).data('razdel')+'_'+$(val).data('user')+'.xml&user='+$(val).data('user')+'&xml&fio='+fio;
...
$('#ttn').append(t); 
  ...
  setTimeout(function() {
    	dat=$('#ttn').prop('outerHTML');


 var BOM = new Uint8Array([0xEF,0xBB,0xBF]);
                    var file = new Blob([BOM,dat], {type: 'text/plain;charset=UTF-8'});
                   // Others
                    saveAs(file, "отчет.xls");
...
//console.log(data);
$('#ttr'+id).after("<tr><td colspan=6><button class='rpc'>печать</button><pre style=' font-size: 10pt;'>"+data+"</pre></td></tr>");
});	});	
	
	$('body').on('click', '.pank',function(e){
		fio=($(this).parent().parent().find('#fio').html());
		e.preventDefault();
		console.log('1');
		razdel=$(this).data('razdel')+'_'+$(this).data('user')+'.xml';
		user=$(this).data('user');
		path=$(this).data('path');
			id=$(this).data('id');
		$.get( "nmo/obr/"+path,{path:razdel,user:user}, function(data) {
//console.log(data);
$('#ttr'+id).after("<tr><td colspan=6><h3>"+fio+"</h3><button class='rpc'>печать</button>"+data+"</td></tr>");
});	});	
