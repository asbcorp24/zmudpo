...
FROM
  `tm_nmo_razd_media`
WHERE
  `tm_nmo_razd_media`.`tm_nmo_razd` =".$row_test['id']; 
	 ...
	 <?php if ($totalRows_sqlu>0) {
	  ...
	  ?>
	     ...
	     <div class="wp-block-content">
            <small>
<span class="glyphicon glyphicon-calendar" aria-hidden="true"></span> <?php echo date("d.m.y");?></small>
            <h4 class="content-title">Тест пройден<span class="glyphicon glyphicon-bookmark pull-right" aria-hidden="true">  правильных ответов <?php echo $row_sqlu['proydeno']; ?></span>  </small></h4>
...
$totalRows_list =  /* fixed MMiC */ mysqli_num_rows($list);  
	  ...
	  Будет доступно после сдачи тестов предыдущего раздела
	  <?php } ?>
      </div>
    </div>
  </div>
