...
$tmp= "<h2 style='position: relative;z-index: 10000;color:red;margin-left:200px'>".$row_nmo1['fio']."-".date("F j, Y, g:i a")."</h2>";
$sql="SELECT 
...
$nmo1 =  /* fixed MMiC */ DB::Query($sql, $testmed) or die( /* fixed MMiC */ mysqli_error(DB::$link));
$row_nmo1 =  /* fixed MMiC */ mysqli_fetch_assoc($nmo1);  
$tmp2= "<h2 style='position: relative;z-index: 10000;color:red;margin-left:200px'>".$row_nmo1['nazv']."</h2>";

$sql="SELECT 
 MAX(`tmo_nmo_test_dat`.`dat`) as dat
 
FROM
  `tmo_nmo_test_dat`
WHERE
  `tmo_nmo_test_dat`.`user` = ".$_SESSION['MM_Username1']." AND 
  `tmo_nmo_test_dat`.`test` =  ".$_SESSION['MM_spec']."
";
  $nmo12=  /* fixed MMiC */ DB::Query($sql, $testmed) or die( /* fixed MMiC */ mysqli_error(DB::$link));
$row_nmo12 =  /* fixed MMiC */ mysqli_fetch_assoc($nmo12);
$totalRows_nmo12 =  /* fixed MMiC */ mysqli_num_rows($nmo12);
 //echo $row_nmo12['dat'];
$now = new DateTime(); // текущее время на сервере
$date = DateTime::createFromFormat("Y-m-d H:i:s", $row_nmo12['dat']); // задаем дату в любом формате
$interval = $now->diff($date); // получаем разницу в виде объекта DateInterval
//$ii=$interval->d*24*60+ $interval->h*60+ $interval->i;
$ii=50;
//echo $sql;echo "<br>";
//echo $ii;echo "<br>";
//echo  $row_nmo12['dat'];echo "<br>";
// $interval->i; // кол-во минут
if ((($ii)<10) and (!is_null($row_nmo12['dat']))){
	
	echo is_null($row_nmo12['dat']);
	
	$text = file_get_contents( 'rano.html');

$text=str_replace('!!tt',$ii,$text);
...
e.preventDefault();
    console.log('hren2');
        ...
        $('div').on('mousedown','.component_base.retake.active', function(e) {
         $('div.bottom_panel').find('.component_base.retake').remove();
    e.stopPropagation();
