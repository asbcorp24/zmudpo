...
use App\Models\{Enrollment,LearningSection,SectionProgress,PracticeAssignment,Submission,FinalWork,QuizAssignment,QuizAttempt};
class ProgramProgressService {
 public function calculate(Enrollment $e): float {
  ...
  $hasFinal=FinalWork::where('enrollment_id',$e->id)->exists();
  $finalPct=$hasFinal?(FinalWork::where('enrollment_id',$e->id)->whereIn('status',['accepted','passed'])->exists()?100:0):100;
  $parts=collect([$sectionPct]);
  if($practiceIds->isNotEmpty())$parts->push($practicePct);
  if($quizIds->isNotEmpty())$parts->push($quizPct);
  if($hasFinal)$parts->push($finalPct);
