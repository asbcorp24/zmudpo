...
class UserController extends Controller {
 ...
 private function data(Request $r,bool $new){return $r->validate(['full_name'=>'required|max:255','login'=>'required|max:255|unique:users,login,'.($new?'NULL':$r->route('user')?->id),'email'=>'nullable|email','role'=>'required|in:student,curator,admin','password'=>$new?'required|min:6':'nullable|min:6'])+['is_active'=>$r->boolean('is_active'),'is_legal_entity'=>$r->boolean('is_legal_entity')];}
