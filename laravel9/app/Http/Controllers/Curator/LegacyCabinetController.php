...
use App\Models\{Announcement,ContentItem,EducationDocument,Enrollment,Group,LearningSection,LearningSectionProgress,QuizAttempt,SectionProgress,Submission,FinalWork,Survey,SurveyResponse};
...
class LegacyCabinetController extends Controller
{
    ...
    private function allowedEnrollment(Request $r,Enrollment $e): void {abort_unless($r->user()->isAdmin()||$e->curator_id===$r->user()->id,403);}
    ...
    public function students(Request $r)
    {
        ...
        if($r->filled('q')){$term=$r->string('q')->toString();$q->whereHas('user',fn($x)=>$x->where(fn($u)=>$u->where('full_name','like','%'.$term.'%')->orWhere('login','like','%'.$term.'%')));}
        $items=$q->orderByDesc('status')->latest('id')->paginate(60)->withQueryString();$ids=$items->pluck('user_id');
    ...
    public function files(Request $r)
    {
        ...
        $finals=FinalWork::with(['user','enrollment.program'])->whereIn('user_id',$ids)->whereNotNull('file_path')->whereHas('enrollment',fn($q)=>$q->whereIn('program_id',$pids))->latest('submitted_at')->get();
        return view('curator.files',compact('submissions','finals'));
    }

    public function content(Request $r)
    {
        $sections=LearningSection::with('contentItems')->whereIn('program_id',$this->programIds($r))->orderBy('program_id')->orderBy('position')->get();
        return view('curator.content',compact('sections'));
    }
    public function updateContent(Request $r,ContentItem $item)
    {
        $item->load('section');abort_unless($r->user()->isAdmin()||$this->programIds($r)->contains($item->section?->program_id),403);
    ...
    {
        $items=EducationDocument::with(['user','program'])->whereIn('user_id',$this->studentIds($r))->whereIn('program_id',$this->programIds($r))->latest('issued_at')->paginate(80);
        return view('curator.documents',compact('items'));
    }

    public function announcements(Request $r)
    {
        $pids=$this->programIds($r);$items=Announcement::with('program')->where(fn($q)=>$q->whereNull('program_id')->orWhereIn('program_id',$pids))->latest('published_at')->paginate(50);
    ...
    public function storeAnnouncement(Request $r)
    {
        ...
        abort_unless($r->user()->isAdmin()||$this->programIds($r)->contains((int)$d['program_id']),403);
    ...
    {
        $ids=$this->studentIds($r);$surveys=Survey::with('fields')->get();$responses=SurveyResponse::whereIn('user_id',$ids)->get()->groupBy('user_id');$students=$this->enrollments($r)->get()->pluck('user','user_id')->filter();
