...
class ImportLegacyCuratorCabinet extends Command
{
 ...
 protected $description='Import historical curator cabinet data: logins, attendance, practices, workbooks, files, chat and announcements';
 public function handle(): int
 {
  $db=DB::connection('legacy');$s=$db->getSchemaBuilder();$tables=['tm_login_dat','nmo_otm_pos','tm_nmo_pract','tm_user_sh','tm_nmo_user_file','tm_chat_kurator','tm_obiav'];
  ...
  if($s->hasTable('tm_login_dat'))$this->loginHistory($db,$users);if($s->hasTable('nmo_otm_pos'))$this->attendance($db,$users);if($s->hasTable('tm_nmo_pract'))$this->practice($db,$users);if($s->hasTable('tm_user_sh'))$this->workbooks($db,$users,$this->option('workbooks-dir')?:base_path('../tetrad/user'));if($s->hasTable('tm_nmo_user_file'))$this->files($db,$users,$this->option('user-files-dir')?:base_path('../usrimg'));if($s->hasTable('tm_chat_kurator'))$this->chat($db,$users);if($s->hasTable('tm_obiav'))$this->announcements($db);return self::SUCCESS;
 }
 private function loginHistory($db,$users): void {$n=0;foreach($db->table('tm_login_dat')->get() as $r){$legacy=(int)($r->user??0);LegacyCuratorRecord::updateOrCreate(['type'=>'login','source_key'=>$this->key($r,['num','id'])],['user_id'=>$users[$legacy]??null,'legacy_user_id'=>$legacy?:null,'occurred_at'=>$this->date($r->dat??null),'body'=>$r->dop??null,'meta'=>(array)$r]);$n++;}$this->info("История входов: {$n}");}
