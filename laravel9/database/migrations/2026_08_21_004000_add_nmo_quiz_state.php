...
return new class extends Migration {
 public function up(): void {
  ...
  Schema::table('quiz_user_overrides',function(Blueprint $t){$t->dropUnique(['quiz_id','user_id']);$t->unsignedBigInteger('legacy_id')->nullable()->index();$t->boolean('is_active_override')->nullable();$t->unique(['quiz_assignment_id','user_id'],'quiz_override_assignment_user_unique');});
 ...
 public function down(): void {Schema::dropIfExists('legacy_quiz_xml_results');Schema::dropIfExists('quiz_attempt_details');Schema::dropIfExists('learning_section_progress');Schema::table('quiz_user_overrides',function(Blueprint $t){$t->dropUnique('quiz_override_assignment_user_unique');$t->dropColumn(['legacy_id','is_active_override']);$t->unique(['quiz_id','user_id']);});Schema::table('quiz_assignments',fn(Blueprint $t)=>$t->dropColumn('legacy_media_id'));}
};
