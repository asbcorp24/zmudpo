...
FROM
  `tm_user_sh`
WHERE
  `tm_user_sh`.`user` =  $id AND 
  `tm_user_sh`.`media` =$media";
...
$totalRows_spec01 =  /* fixed MMiC */ mysqli_num_rows($spec01);
	$prepod=-1;
//	if (isset($_SESSION['MM_Username2020']))$prepod=$_SESSION['MM_Username2020'];
//	echo "-------------->";
				$zip = new ZipArchive();
	
			$zip = zip_open('./tetrad/user/'.$row_spec01['path'].".zip");
			if ($zip) {
    while ($zip_entry = zip_read($zip)) {
         ...
         $html = str_get_html($buf);
         print_r($html);
        if ($html==''){echo $buf;  zip_entry_close($zip_entry);exit;}
         exit;
foreach($html->find("input") as $a){
	if ($a->type=="checkbox") $a->outertext = "<span align=center>". $a."</span>"; else
    $a->outertext = "<span>". $a->value."</span>";
    
}   
foreach($html->find("button") as $a){

    $a->outertext = "<span></span>";
    
}  
foreach($html->find("textarea") as $a){
...
<div class="panel-group" id="accordion">
  <!-- 1 панель -->
	  <?php do { ?>
  <div class="panel panel-default fuser" id="pan<?php echo $row_spec01['num']; ?>" data-id="grp<?php echo $row_spec01['nazv']; ?>">
    <!-- Заголовок 1 панели -->
    <div class="panel-heading">
      <h4 class="panel-title">
        <a data-toggle="collapse" data-parent="#accordion" href="#collapse<?php echo $row_spec01['num']; ?>"><?php echo $row_spec01['fio']; ?></a><span class="badge pull-right">  <?php echo $row_spec01['nazv']; ?></span>
      </h4>
    </div>
    <div id="collapse<?php echo $row_spec01['num']; ?>" class="panel-collapse collapse bfd" data-id="<?php echo $row_spec01['num']; ?>" data-media="<?php echo $row_spec01['id']; ?>">
      <!-- Содержимое 1 панели -->
...
<div class="btn-group pull-left">
	<div><select style="max-width: 200px" id="srazd">
      ...
      <?php do { ?>
         <option value="<?php echo $row_spec02['id']; ?>"><?php echo getsokr($row_spec02['nazv']); ?>[<?php echo $row_spec02['rnazv']; ?>]</option>
...
<script src="js/bootstrap.js"></script>
 ...
 $('body').on('show.bs.collapse', '.bfd', function(){
   id=$(this).data('id');
	...
	$.post( "kurator_sh.php",{'get':1,'id':id,'media':media}, function(data) {
$('#fss'+id).html(data);
...
id=$('#srazd').val();
$.post( "kurator_sh.php",{'fio':1,'id':id}, function(data) {
$('#todo').html(data);
